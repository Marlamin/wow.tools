...
class BLTEIntegrityError extends Error {
	constructor(expected, actual) {
		console.log('[BLTE] Invalid block data hash. Expected %s, got %s!', expected, actual);
...
class BLTEReader  {
	...
	constructor(buf, hash, partialDecrypt = false) {
		...
		if (hashCheck !== hash)
			throw new Error("[BLTE] Invalid MD5 hash, expected " +  hash + " got " + hashCheck)
	...
	_decryptBlock(data, blockEnd, index) {
        console.log("TACT encryption NYI");
        return;
...
var BLTEReader = require("./blte-reader");

    let wowDirectoryHandle;
            let buildInfo = [];
            let indexDict = new Map();

            
            async function openDir(){
                wowDirectoryHandle = await window.showDirectoryPicker();
                await listDir(wowDirectoryHandle);
            }

            var button = document.getElementById('openDirButton'); // add id="my-button" into html
button.addEventListener('click', openDir);

            async function openFileFD() {
                fileHandle = await window.showOpenFilePicker();
                const file = await fileHandle[0].getFile();
                const fileBuffer = await file.arrayBuffer();
                const fileBytes = new BLTEReader(fileBuffer);
                fileBytes.processAllBlocks();
                console.log(fileBytes);
                let blp = new BLPFile(fileBytes.getOutputBuffer());
                let canvas = document.getElementById("output");
                let image = blp.getPixels(0, canvas, 512, 512);
            }

            async function listDir(directoryHandle){
                for await (let [name, handle] of directoryHandle) {
                    if (handle.kind == "file" && name == ".build.info") {
                        console.log("Found .build.info");
                        readBuildInfo(handle);
                        break;
                    }
                }
            }

            async function readBuildInfo(handle){
                ...
                for(let i = 1; i < lines.length; i++){
                    ...
                    for(let j = 0; j < segments.length; j++){
                        buildInfo[i - 1][headers[j].split('!')[0]] = segments[j];
                    }
                }

                for(let i = 0; i < buildInfo.length; i++){
                    writeToLog("Found " + buildInfo[i]["Product"] + " install of version " + buildInfo[i]["Version"] + " (buildconfig " + buildInfo[i]["Build Key"] + ", cdnconfig " + buildInfo[i]["CDN Key"] +") <a href='#' id='loadBuild" + i + "' data-build='" + i + "''>Load</a>");
                    document.getElementById("loadBuild" + i).addEventListener('click', loadBuildInfoEntry);
                }
            }

            async function loadBuildInfoEntry(e){
                entry = e.currentTarget.dataset.build;
                writeToLog("Loading build config " + buildInfo[entry]["Build Key"] + " with CDN config " + buildInfo[entry]["CDN Key"]);
                const buildConfigHandle = await openFile(hashToDir(buildInfo[entry]["Build Key"], "config"));
                const buildConfigFile = await buildConfigHandle.getFile();
                const buildConfigText = await buildConfigFile.text();
                const buildConfig = parseConfig(buildConfigText);
                
                console.log(buildConfig);

                const cdnConfigHandle = await openFile(hashToDir(buildInfo[entry]["CDN Key"], "config"));
                const cdnConfigFile = await cdnConfigHandle.getFile();
                const cdnConfigText = await cdnConfigFile.text();
                const cdnConfig = parseConfig(cdnConfigText);

                const indexDirHandle = await openDirectory("Data/data");
                let indexCount = 0;
                for await (let [name, handle] of indexDirHandle){
                    if(name.endsWith(".idx")){
                        await parseIndex(handle);
                        indexCount++;
                    }
                }

                writeToLog("Parsed " + indexCount + " indices, got " + indexDict.size + " entries");

                writeToLog("Loading encoding " + buildConfig.encoding[1]);
                const encodingFileBytes = await getFileFromArchives(buildConfig.encoding[1]);
                const encodingFile = new BLTEReader(encodingFileBytes);
                encodingFile.processAllBlocks();
                writeToLog("Loaded encoding!");

                // There are issues in blte.js that I badly ported
                console.log(encodingFile);
            }

            function hashToDir(hash, type){
                return "Data/" + type + "/" +  hash[0] + hash[1] + "/" + hash[2] + hash[3] + "/" + hash;
            }

            async function openDirectory(dirname) {
                const parts = dirname.split("/");
                let dirHandle = wowDirectoryHandle;
            ...
            function writeToLog(line){
                document.getElementById("log").insertAdjacentHTML('beforeend', line + "\n");
            }
            ...
            function parseConfig(lines){
                ...
                for(let line of lines){
                    ...
                    const keyName = splitLine[0];
                    let keyValues = splitLine[1];

                    if(keyValues.includes(" ")){
                        keyValues = keyValues.split(" ");
                    }

                    config[keyName] = keyValues;
                }

                return config;
            }

            async function parseIndex(fileHandle) {
                const file = await fileHandle.getFile();
                const index = new Bufo(await file.arrayBuffer());
                ...
                for (let i = 0; i < nBlocks; i++) {
                    ...
                    indexDict.set(key, {
                        ...
                        size: index.readInt32()
                    });
                }
            }

            async function getFileFromArchives(key){
                if(indexDict.has(key.substring(0, 18))){
                    console.log(indexDict.get(key.substring(0, 18)))
                    const file = indexDict.get(key.substring(0, 18));
                    const archiveHandle = await openFile("Data/data/data." + file["index"].toString().padStart(3, '0'));
                    const archiveFile = await archiveHandle.getFile();
                    const archive = new Bufo(await archiveFile.arrayBuffer());
                    console.log(archive);
                    archive.seek(file.offset + 30);
                    const fileBuffer = archive.readArrayBuffer(file.size - 30);
                    return fileBuffer;
                }else{
                    console.log("File " + key + " not in local indices");
                }
            }
},{"./blte-reader":4}]},{},[5])(5)
});
