...
foreach($products as $code => $product){
	...
	foreach($pcs as $pc){
		...
		foreach($pc['patch-entry'] as $pe){
			...
			if($type == "partial-priority") continue;
			...
			if($existingBuild['complete'] == 1){
				...
				if(!doesFileExist("config", $row['buildconfig'], $product['cdndir'])) {
					...
					foreach($bcrow as $key => $value){
						if(!empty($value)){
							if($key == "unarchived" || $key == "unarchivedcount" || $key == "unarchivedcomplete") continue;
...
foreach($missingfiles as $missingfile){
	if(in_array($missingfile['filename'], $missingfilesindb)) continue;
