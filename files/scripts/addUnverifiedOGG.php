...
while ($row = $q->fetch()) {
    ...
    if (substr($row['name'], 0, 3) == "vo_") {
        ...
        if (strpos($row['name'], '_pc_') !== false) {
            ...
            if ($isNumericVO) {
                ...
                $files[$row['filedataid']] = "sound/creature/" . $cleaned . "/" . $row['name'] . ".ogg";
            } else {
    ...
    } elseif (substr($row['name'], 0, 4) == "mon_") {
        // mon
    ...
    } elseif (substr($row['name'], 0, 4) == "amb_") {
        // zone ambience
    } elseif (substr($row['name'], 0, 5) == "spell") {
        // spell sounds
    } elseif (substr($row['name'], 0, 4) == "mus_") {
        // muzak
    } elseif (substr($row['name'], 0, 3) == "fx_") {
        // fx
        for ($i = 0; $i < 99; $i++) {
    ...
    } elseif (substr($row['name'], 0, 3) == "go_") {
        // gameobject sounds
        for ($i = 0; $i < 99; $i++) {
