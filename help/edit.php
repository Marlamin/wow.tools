...
<div class='container-fluid'>
    <div class='row'>
        <div class='col-md-6'>
            <form action='/help/edit.php<?php if (!empty($tutorial['id'])) {
                echo "?id=" . $tutorial['id'];
                                        }?>' method='POST'>
                <?php if (!empty($_GET['id'])) {
                    ?><input type='hidden' name='id' value='<?=$tutorial['id']?>'><?php
                } ?>
                <label for='name'>Title: </label> <input class='form-control' id='name' type='text' name='name' value='<?php if (!empty($tutorial['name'])) {
                    echo $tutorial['name'];
                                                                                                                       } ?>'><br>
                <textarea id="source" rows="25" cols="100" name='content'><?php if (!empty($tutorial['content'])) {
